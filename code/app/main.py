...
class DataService:
    ...
    @staticmethod
    def get_book_by_id(product_id: int):
        conn = get_connection()
        try:
            cur = conn.cursor(dictionary=True)
            cur.execute(
    ...
    @staticmethod
    def check_book_exists_in_orders(product_id: int) -> bool:
        conn = get_connection()
        try:
            cur = conn.cursor()
...
class ProductFormDialog(QDialog):
    ...
    def load_product(self, product_id: int):
        row = DataService.get_book_by_id(product_id)
        if not row:
...
class ProductsWindow(QMainWindow):
    def __init__(self, user_data: UserInfo):
        ...
        if os.path.exists(ICON_PNG):
            ...
            logo.setPixmap(pix)
        title = QLabel("ЧитайГород — Список товаров")
    ...
    def load_products(self):
        self.products = DataService.get_books()
        self.apply_filters()
    ...
    def delete_book(self):
        ...
        if not product_id:
            ...
            return
        if DataService.check_book_exists_in_orders(product_id):
        ...
        if ans != QMessageBox.StandardButton.Yes:
            return
        try:
            DataService.delete_book(product_id)
        except Exception as ex:
            show_error(self, f"Ошибка удаления:\n{ex}")
            return
        self.load_suppliers()
